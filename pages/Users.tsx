...
const Users: React.FC = () => {
  ...
  const [selectedNetworkUser, setSelectedNetworkUser] = useState<User | null>(
    null
  );
  ...
  const fetchNetwork = async (user: User) => {
    ...
    setNetworkData([]);
    try {
      const res = await apiRequest(ENDPOINTS.MY_AFFILIATE, "GET", undefined, {
    ...
    () => [
      {
        ...
        header: "",
        render: (_, item) => (
          <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-200 dark:border-slate-700 bg-gray-50">
            <img
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <Circle
                ...
                className={`fill-current ${
                  isUserActive(item) ? "text-emerald-500" : "text-gray-300"
                }`}
              />
      ...
      },
      {
        ...
        render: (_, item) => {
          ...
          return (
            <span
              className={`px-2 py-1 rounded-lg text-xs font-bold border flex items-center gap-1 w-fit ${color}`}
            >
        ...
        render: (val, item) =>
          val ? (
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleFilterByParent(String(val));
              }}
        ...
        render: (_, item) => (
          <div className="flex gap-2 justify-end">
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold transition-colors border ${
                item.is_verified === 0
            ...
            >
              ...
              <span className="hidden sm:inline">
                {item.is_verified === 0 ? t("unfreeze") : t("freeze")}
              </span>
            </button>
          </div>
        ),
        ...
        <div className="flex gap-2 overflow-x-auto">
          {["all", "active", "inactive", "frozen"].map((f) => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors border-b-2 whitespace-nowrap capitalize ${
                currentFilter === f
            ...
            >
              {t(
                (f === "active"
                  ...
                  : f === "frozen"
                  ? "frozen_users"
                  : "all") as keyof (typeof translations)["en"]
              )}
            </button>
          ))}
        </div>
          ...
          <div className="relative">
            ...
            <div
              className={`absolute top-1/2 -translate-y-1/2 pointer-events-none text-gray-400 ${
                document.dir === "rtl" ? "left-2" : "right-2"
              }`}
            >
              {sortOrder === "newest" ? (
                <SortDesc size={14} />
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-900/50 rounded-t-2xl">
              <div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                  {networkHistory.length > 0 && (
                    <button
                      ...
                      className="mr-2 p-1 rounded-full hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors"
                    >
                      <ArrowUpLeft
                        size={20}
                ...
                <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">
                  ...
                  Code:{" "}
                  <span className="font-mono bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 px-1.5 py-0.5 rounded">
                    {selectedNetworkUser?.affiliate_code}
                  </span>
                </p>
              </div>
              <button
                onClick={closeNetworkModal}
                className="text-gray-400 hover:text-gray-600 dark:hover:text-slate-200 transition-colors p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full"
              >
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-4 border-t border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-900/50 rounded-b-2xl flex justify-end">
              <button
                onClick={closeNetworkModal}
                className="px-6 py-2 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors"
              >
                {t("close")}
              </button>
            </div>
